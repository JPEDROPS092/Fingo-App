...
export default function List01({ className }: Partial<List01Props>) {
  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      try {
        ...
        const accountsData = accountsResponse.data.results.map((account: any) => ({
          id: account.id,
          title: account.title,
          description: account.description || undefined,
          balance: `$${parseFloat(account.balance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
          type: account.type,
        }));
        setAccounts(accountsData);
        
        // Fetch total balance
        const balanceResponse = await accountsService.getTotalBalance();
        setTotalBalance(`$${parseFloat(balanceResponse.data.total_balance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
      } catch (error) {
        console.error('Failed to fetch accounts:', error);
        toast.error('Failed to load accounts');
        // Fallback to demo data if API fails
        setAccounts(ACCOUNTS);
        setTotalBalance("$26,540.25");
      } finally {
        setLoading(false);
      }
    };
